...
<ul style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; padding: 10px; list-style-type: none; font-family: Arial, sans-serif;">
    <?php foreach($jezdci as $rider): ?>
        <li style="border: 1px solid #e0e0e0; padding: 12px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); background-color: #fff; font-size: 14px; display: flex; gap: 12px; align-items: flex-start;">
            
            <div style="flex-shrink: 0;">
                <?php if (!empty($rider['photo'])): ?>
                    <img src="/kec/projektcyklo/cykloProjekt/obrazky/riders/<?= $rider['photo'] ?>" alt="<?= $rider['last_name'] ?>" style="width: 70px; height: 90px; object-fit: cover; border-radius: 4px; border: 1px solid #ccc;">
                <?php else: ?>
                    <div style="width: 70px; height: 90px; background-color: #eee; border-radius: 4px; border: 1px solid #ddd; display: flex; align-items: center; justify-content: center; color: #aaa; font-size: 11px;">No foto</div>
                <?php endif; ?>
            </div>
            ...
            <div style="flex-grow: 1; min-width: 0;">
                <h3 style="margin-top: 0; margin-bottom: 8px; color: #0056b3; font-size: 16px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center; gap: 8px;">
                    ...
                    <?php if (!empty($rider['country'])): ?>
                        <span class="fi fi-<?= strtolower($rider['country']) ?>" style="border-radius: 2px; flex-shrink: 0;"></span>
                    <?php endif; ?>
                </h3>
